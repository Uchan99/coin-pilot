...
<<<<<<< HEAD
## 현재 운영 상태 요약 (2026-03-12 기준)
=======
## 현재 운영 상태 요약 (2026-03-14 기준)
...
<<<<<<< HEAD
- LLM 비용 관측(21-04): 개인 계정 fallback 기준 `blocked`
  - `llm_usage_events` 기반 내부 usage observability는 운영 중
  - `llm_provider_cost_snapshots` 기반 provider reconciliation은 org/admin 계정 capability 확보 전까지 보류
=======
- LLM 비용 관측(21-04): 개인 계정 fallback 기준 `done`
  - `llm_usage_events` 기반 내부 usage observability는 운영 중
  - `llm_provider_cost_snapshots` 기반 provider reconciliation은 org/admin 계정 capability 확보 전까지 후속 범위로 분리
>>>>>>> 4a1862d (docs(ops): sync OCI verification results and realign 21-04/23 gates)
  - 운영 집계 스크립트: `scripts/ops/llm_usage_cost_report.sh`
- 운영 점검 자동화(31): 완료(`done`)
  - `/etc/cron.d/coinpilot-monitoring` + `scripts/ops/run_scheduled_monitoring.sh` 표준 적용
  - `check_24h_monitoring.sh`에 `LLM snapshot freshness`, `AI decision inactivity`, `scheduled heartbeat` 자동 판정 추가
  - 2026-03-10 OCI 검증: `all --automation-mode` 결과 `FAIL:0/WARN:0`
- OCI 원격 접근 표준(31-02): 완료
  - WSL에서 `scripts/ops/oci_remote_exec.sh`, `scripts/ops/oci_tunnel.sh`로 OCI 운영 명령/브라우저 접근을 표준화
  - 설정 예시: `deploy/cloud/oci/ops/oci_access.env.example`
  - 원칙: 운영 source of truth는 OCI이며, 로컬 Docker 유무와 무관하게 wrapper를 통해 OCI를 우선 사용
- 전략 평가(29): 완료(`done`)
  - `market_data` 실가용 범위는 심볼별 약 `122.17 ~ 124.80일`
  - `transition_sensitive`는 baseline 대비 손실/MDD를 줄였지만 `avg_profit_per_trade < 0`, `Reward/Risk < 1.0`, `Profit Factor < 1.0` 상태라 추가 레짐 완화 핫픽스는 보류
  - 현재 baseline에는 심볼 재배분 정책(BTC/ETH/SOL 1.2, XRP/DOGE 0.7)이 이미 반영된 상태
...
- `scripts/ops/run_scheduled_monitoring.sh`: cron/systemd용 표준 래퍼(`flock`, `timeout`, 일단위 로그)
...
- `LLM_COST_SNAPSHOT_ENABLED=true|false` (개인 계정 기본 `false`)
...
=======
2. `21-04` LLM 토큰/비용 관측 대시보드 (`done`)
>>>>>>> 4a1862d (docs(ops): sync OCI verification results and realign 21-04/23 gates)
