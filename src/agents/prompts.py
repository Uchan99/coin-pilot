...
[당신이 판단해야 할 것]
1. 최근 캔들 구조에서 비정상적 변동성이 보이는지 (급등/급락 캔들, 장대 음봉 연속, 위꼬리 급등 후 반락)
2. 급격한 가격 변동이 패닉 셀링/FOMO 매수와 연관되는지
3. Head Fake 패턴 (짧은 반등 후 즉시 하락 전환) 징후가 있는지
4. 투자자의 연속 손실 상태에서의 뇌동매매(Revenge Trading) 가능성

[캔들 구조 분석 가이드]
아래 OHLC 데이터가 제공되면, 다음을 확인하세요:
...
[판단 기준]
- SAFE: 특별한 거시적 위험 신호가 없으며, 캔들 구조가 안정적인 경우.
- WARNING: 시장 변동성이 비정상적이거나, 패닉/FOMO 징후가 명확하거나, 캔들 구조에서 급락/Head Fake 패턴이 감지된 경우.
"""
