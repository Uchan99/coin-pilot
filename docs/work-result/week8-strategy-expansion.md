...
## 추가 수정사항 (v2.5 듀얼 RSI 전략)

> **수정일**: 2026-02-06
> **수정자**: Claude Code
> **사유**: 하락장에서 단순 RSI 조건으로는 시그널 발생 불가 → 듀얼 RSI + MA 완화 전략 도입

### 문제 분석

기존 v2.4 전략의 한계:
```
RSI(14) < 40      → 과매도 상태 확인 (OK)
Price > MA(20)    → 하락장에서 충족 불가 (문제!)
→ 시그널 발생 0건
```
...
| 항목 | v2.4 | v2.5 | 변경 근거 |
|------|------|------|-----------|
| **RSI(14)** | < 40 | < **40** | 유지 (중기 과매도 확인) |
| **RSI(7)** | 없음 | **30 상향 돌파** | 반등 모멘텀 시점 포착 |
| **MA 조건** | Price > MA(20) | Price ≥ MA(20) × **0.97** | MA 근처 진입 허용 |
| **Volume** | > 1.2x | > **1.2x** | 유지 |
...
| 파일 | 수정 내용 |
|------|-----------|
| `src/config/strategy.py` | `RSI_SHORT_PERIOD`, `RSI_SHORT_CROSSOVER`, `MA_TOLERANCE` 추가 |
| `src/common/indicators.py` | `rsi_short`, `rsi_short_prev` 계산 추가 |
| `src/engine/strategy.py` | 듀얼 RSI + MA tolerance 진입 조건 구현 |
| `src/bot/main.py` | `build_status_reason()` RSI7 상태 표시 추가 |
| `scripts/backtest_signal_count.py` | v2.5 전략 백테스트 지원 |

### 백테스트 결과

```
============================================================
시그널 발생 비교: v2.4 vs v2.5 듀얼 RSI 전략
============================================================
KRW-BTC:  v2.4: 0건 → v2.5: 129건
KRW-ETH:  v2.4: 0건 → v2.5: 40건
KRW-XRP:  v2.4: 0건 → v2.5: 42건
KRW-SOL:  v2.4: 0건 → v2.5: 42건
KRW-DOGE: v2.4: 0건 → v2.5: 5건
------------------------------------------------------------
총계: 0건 → 258건 (시그널 발생!)
```
...
┌─────────────────────────────────────────────────────────────┐
│  1. RSI(14) < 40                                            │
│     → 중기 과매도 상태 확인 (충분히 하락함)                    │
│                                                             │
│  2. RSI(7)가 30 아래에서 30 위로 상향 돌파                    │
│     → 단기 반등 모멘텀 시작 (바닥 확인)                       │
│                                                             │
│  3. Price ≥ MA(20) × 0.97                                   │
│     → MA선 3% 이내 근처면 진입 허용 (하락장 적응)              │
│                                                             │
│  4. Volume > 1.2x                                           │
│     → 거래량 동반 확인 (진짜 반등인지)                        │
└─────────────────────────────────────────────────────────────┘
         ↓
   "과매도 상태에서 반등하는 순간"을 포착
```
...
```python
# src/config/strategy.py
USE_CONSERVATIVE_MODE = True  # 즉시 BTC only, RSI30, MA200으로 복귀
```

---

*v2.5 Dual RSI Strategy by Claude Code*
