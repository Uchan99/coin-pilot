...
- 2026-03-15: OCI 24h/72h 실측 결과를 반영했다. canary env와 OpenAI 경로는 정상 활성화됐고 비용/지연 이점도 확인됐지만, 24h 기준 canary 표본은 `3`건으로 최소 기준 `N>=20`에 크게 못 미쳐 상태는 계속 `in_progress`로 유지한다.
