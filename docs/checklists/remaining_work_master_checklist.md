...
| 6 | 28 | AI Decision 전략문서/과거사례 기반 RAG 보강 | in_progress | 21-03/21-04 관측 데이터와 승인 확보 | 전략 문서 + 과거 사례 우선 RAG의 offline replay 비교 완료 + live canary 제한 주입 기준 확정 | `PYTHONPATH=. .venv/bin/pytest -q tests/agents/test_ai_decision_rag.py` + `python3 -m py_compile src/agents/ai_decision_rag.py src/agents/ai_decision_replay.py src/agents/analyst.py src/agents/state.py src/agents/runner.py scripts/replay_ai_decision_rag.py` + `bash -n scripts/ops/replay_ai_decision_rag.sh` + `PYTHONPATH=. .venv/bin/python scripts/replay_ai_decision_rag.py --help` | `docs/work-plans/28_ai_decision_strategy_case_rag_plan.md` | `docs/work-result/28_ai_decision_strategy_case_rag_result.md` |
...
- 2026-03-11: `28` 사용자 승인 후 착수(`in_progress`). live canary 표본 부족을 반영해 Phase 1을 offline replay로 전환했고, 전략/사례 2계층 RAG helper(`src/agents/ai_decision_rag.py`), BUY `signal_info` 기반 replay 샘플 복원기(`src/agents/ai_decision_replay.py`), replay CLI/ops 래퍼, Analyst 선택적 RAG 주입 경로를 구현했다. 로컬 검증은 신규 테스트 `4 passed`, `py_compile`, shell syntax, CLI help 통과까지 완료했으며 실제 replay 실행은 다음 단계에서 수행한다.
