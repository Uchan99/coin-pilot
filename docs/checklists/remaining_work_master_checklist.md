...
| 10 | 34 | AdaptiveMeanReversion 5단계 전략 개선 (Phase 1~4 배포: 동시2개+30%+BB_MIDLINE_EXIT+Analyst v3.5.1+Guardian OHLC, Phase 5 대기) | in_progress | 사용자 승인 (2026-03-27), Phase 1~2 배포 (2026-03-28), Phase 3 배포 (2026-03-29), Phase 4 배포 (2026-03-30) | 동시 3심볼 SL 재발 없음 + TIME_LIMIT avg_loss 개선 + boundary_violation 20% 이하 + Guardian reasoning 캔들 구조 언급 + **Guardian blocked 비율 추적 (분리 구조 가치 판단 핵심)** | backtest_v3.py 비교 + boundary_violation SQL + Guardian blocked SQL + 실운영 2주 후 SQL 검증 | `docs/work-plans/34_adaptive_mean_reversion_4stage_improvement_plan.md` | `docs/work-result/34_adaptive_mean_reversion_5stage_improvement_result.md`, `docs/troubleshooting/strategy-retrospectives/34_rr_inversion_and_correlated_loss_clustering.md` |
| 11 | 33 | 전략 청산 파라미터 튜닝 (Stop Loss 축소 + TIME_LIMIT 조정) | done | 사용자 승인 | SIDEWAYS avg PnL +0이상, STOP_LOSS 평균 -2.5% 이내 | 동일 분석 쿼리 2주 후 재실행 | `docs/work-plans/33_strategy_exit_parameter_tuning_plan.md` | - | 사용자 승인 + 저장소 전수 검토 범위 확정 | `docs/portfolio/study/32_*` 개인 학습용 비공개 문서 + index/result 작성 완료, 전수 검증 근거 확보 | `rg -n "docs/portfolio/study/32_|개인 학습용 비공개|private study" docs/work-plans/32_project_end_to_end_runbook_documentation_plan.md docs/work-result/32_project_end_to_end_runbook_documentation_result.md` + 결과 문서의 coverage 검증 섹션 확인 | `docs/work-plans/32_project_end_to_end_runbook_documentation_plan.md` | `docs/work-result/32_project_end_to_end_runbook_documentation_result.md` |
...
| 11 | 35 | 다중 근거 기술적 분석(Multi-Evidence TA) 진입 체계 도입 (Phase 1 백테스트 완료: FVG 압도적 우수, R:R/HTF 비효과적. Phase 1-2 구조적 청산+포지션 사이징 역산 백테스트 대기) | in_progress | 사용자 승인 (2026-03-31), 34번 Phase 3/4 모니터링 병행 | FVG 필터 Rule Engine 적용 + 포지션 사이징 역산 내재화 + 트레일링 스탑 고도화 + AI 청산 보류 | `docker exec coinpilot-bot python scripts/backtest_v3.py --multi-evidence` | `docs/work-plans/35_multi_evidence_technical_analysis_plan.md` | `docs/work-result/35_multi_evidence_technical_analysis_result.md` |

---

## 3) 최근 업데이트 로그
- 2026-03-31: `35` 신규 등록. Phase 1 백테스트 완료 — FVG 필터 승률 50.5%→65.2%, STOP_LOSS 85% 감소로 압도적 우수. R:R/HTF 비효과적. 3차 피드백 반영: AI 청산 전면 보류, 포지션 사이징 역산 선행 필요. Phase 1-2 (구조적 청산 백테스트) 대기.
- 2026-03-30: `34` Phase 4 배포 완료. Guardian OHLC 캔들 컨텍스트 전달 — Head Fake/급락 감지 강화. Phase 3 모니터링: boundary_violation 0.0% 달성, SIDEWAYS 샘플 부족으로 계속 모니터링. **Phase 4 핵심 지표: Guardian blocked 비율 (Analyst-Guardian 분리 구조 가치 판단용)**.
- 2026-03-29: `34` Phase 3 배포 완료. Analyst 프롬프트 v3.5.1 — 금지어→허용어 방식, 레짐 추상화, 캔들 패턴 기준 구체화. Phase 1~2 모니터링 정상 확인 (BB_MIDLINE_EXIT 발동, RSI 우선순위 정상, 동시 2개 제한 정상). Phase 3 모니터링 중 (boundary_violation < 20% 확인 예정).
- 2026-03-28: `34` Phase 1~2 OCI 배포 완료. 동시 포지션 5→2, 포지션 사이즈 20%→30%, BB_MIDLINE_EXIT 1.0% 가드 + RSI 우선순위 변경. Result/전략회고 문서 작성.
- 2026-03-25: `33` 신규 등록. 1개월 실운영 데이터 분석 결과 R:R 역전(-0.22%/거래) 확인. SIDEWAYS stop_loss 4%→2%, time_limit 48h→24h 조정 계획, 승인 대기.
- 2026-03-27: `34` 신규 등록. 1개월 실운영 분석 결과 STOP_LOSS avg vol_ratio=38.14 확인 → 4단계 개선 계획 수립 (vol_surge 필터→MA20 exit→Analyst 프롬프트→Guardian OHLC). 승인 대기.
- 2026-03-27: `33` 상태를 `done`으로 전환. 3가지 파라미터 튜닝(SL%, min_profit%, rsi_overbought%) 백테스트 결과 개선 미미 → "변경 없음, 현재 파라미터 유지" 결론.
