...
| 10 | 34 | AdaptiveMeanReversion 4단계 전략 개선 (vol_surge 필터 + MA20 exit + Analyst 프롬프트 + Guardian OHLC) | todo | 사용자 승인 | STOP_LOSS 비율 감소 + boundary_violation 20% 이하 + Guardian reasoning 품질 향상 | backtest_v3.py 비교 + 실운영 2주 후 SQL 검증 | `docs/work-plans/34_adaptive_mean_reversion_4stage_improvement_plan.md` | - |
