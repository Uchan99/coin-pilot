...
관련 계획: `docs/work-plans/18-12_wsl_oci_local_cloud_operations_master_runbook_plan.md`, `docs/work-plans/18-13_oci_24h_monitoring_checklist_plan.md`, `docs/work-plans/18-14_oci_24h_monitoring_script_automation_plan.md`, `docs/work-plans/31-02_oci_remote_ops_access_standardization_plan.md`  
관련 결과: `docs/work-result/18-12_wsl_oci_local_cloud_operations_master_runbook_result.md`, `docs/work-result/18-13_oci_24h_monitoring_checklist_result.md`, `docs/work-result/18-14_oci_24h_monitoring_script_automation_result.md`, `docs/work-result/31-02_oci_remote_ops_access_standardization_result.md`  
...
### 2.4 WSL -> OCI CLI 표준 접근(wrapper)
로컬 WSL에서 운영 명령을 실행할 때는 "docker가 로컬에 있는지"를 전제로 하지 말고, 아래 wrapper로 OCI를 source of truth 실행 환경으로 사용한다.
...
필수 설정:
1. `deploy/cloud/oci/ops/oci_access.env`에 `OCI_REMOTE_SSH_TARGET` 또는 `OCI_REMOTE_HOST`/`OCI_REMOTE_USER` 입력
2. `OCI_REMOTE_SSH_KEY_PATH`를 실제 private key 경로로 수정
3. 운영 repo root는 기본값 `/opt/coin-pilot` 유지
...
운영 원칙:
1. `scripts/ops/*.sh`, `docker compose`, `docker exec`, 운영 SQL 조회는 가능하면 wrapper를 통해 OCI에서 실행한다.
2. 로컬 Docker/WSL integration은 선택 사항이고, 운영 source of truth는 OCI다.
3. destructive 명령은 wrapper 기본 예시에 넣지 않는다.

### 2.5 WSL -> OCI UI 표준 접근(wrapper)
대시보드/Grafana/n8n/Prometheus 브라우저 접근도 수동 `ssh -L` 대신 wrapper를 우선 사용한다.
...
```bash
cd /home/syt07203/workspace/coin-pilot
scripts/ops/oci_remote_exec.sh systemctl status coinpilot-compose.service --no-pager
scripts/ops/oci_remote_exec.sh --raw 'cd /opt/coin-pilot/deploy/cloud/oci && docker compose --env-file .env -f docker-compose.prod.yml ps'
```
...
4. Docker API mismatch 대응(2026-03-06 핫픽스)
