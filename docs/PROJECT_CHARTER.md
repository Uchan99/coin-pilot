...
- Major 구현이 완료되면(메인 계획 `done` 또는 아키텍처/운영정책 변경 포함) 같은 변경 묶음에서 `README.md`를 최신 상태로 동기화한다.
...
- 프로젝트 본질과 직접 무관한 관리/메타/실험성 문서 작업은 `99-` prefix를 사용한다.
...
5) Quantified evidence is mandatory in Result/Troubleshooting
- Result/Troubleshooting에는 "무슨 문제를 해결했는지"를 구체적으로 기록한다.
  - 최소 포함: 증상, 영향, 재현 조건, root cause
- 기존 방식 대비 개선은 반드시 정량 수치로 증빙한다.
  - 최소 포함: before/after, 변화량(절대값/퍼센트), 측정 기간, 표본 수, 성공/실패 기준
  - 증빙 출처: SQL/로그/대시보드/스크립트 명령 중 1개 이상
- 정량 측정이 불가한 경우에도 "측정 불가 사유 + 대체 지표 + 추후 측정 계획"을 필수 기재한다.

6) Korean comments for non-trivial code are required
...
* **AI 호출 보호 장치:**
    ...
    * AI Decision 카나리 라우팅(신호 해시 기반, 기본 10%, 최대 20%, 미설정/키 누락 시 primary fallback)
* **보조 Agent:**
    * **SQL Agent:** 자연어 질의 → SQL 변환 → 지표 조회
    * **RAG Agent:** 문서/규칙 검색, 리스크 이벤트 감지
    * **Volatility Model:** GARCH 기반 변동성 예측 → 포지션 사이징

### 2.3 Dashboard UI / Freshness 운영 표준 (v3.3-2, 22)
운영 대시보드는 프론트 기술 스택과 무관하게 아래 계약을 따라야 합니다. 목적은 "지금 보이는 값이 실시간 판단용인지, 지연 상태인지, 수동 재확인 전용인지"를 즉시 구분하게 만드는 것입니다.

**정보 우선순위 계층**
| 계층 | 의미 | 예시 |
| :--- | :--- | :--- |
| **Primary** | 실시간 운영 판단에 직접 쓰는 핵심 지표 | 잔고/평가액, 포지션, Risk 상태, bot regime/action |
| **Secondary** | 맥락/추세 해석용 보조 정보 | 차트, 거래 이력 요약, Exit 분석 집계 |
| **Diagnostic** | 장애 원인분해/실험 관측/운영 설명용 정보 | 시스템 연결상태, agent_decisions, risk audit, monitoring-only 실험 지표 |

**필수 freshness 계약**
1. 화면 또는 데이터 블록은 `last_updated_at`, `data_age_sec`, `stale_threshold_sec`, `freshness_status`를 해석 가능하게 표시해야 한다.
2. `freshness_status`는 `fresh | delayed | stale | failed | manual-only`만 사용한다.
3. DB 기반 일반 조회는 기본적으로 `ttl=30s` 캐시를 고려해 `stale_threshold_sec`를 60초 이상으로 잡는다.
4. Redis bot status처럼 실시간성이 더 중요한 데이터는 별도 기준을 둘 수 있으나, ad-hoc 숫자가 아니라 문서화된 threshold를 사용해야 한다.
5. `21-03`, `28`처럼 monitoring-only 상태의 실험 지표는 Primary가 아니라 Diagnostic 영역에 배치한다.
6. `21-10`처럼 수동 재확인 전용 검증 항목은 `manual-only`로 표기하고 자동 새로고침 핵심 카드에 섞지 않는다.

**상태 해석 규칙**
| 상태 | 의미 | 기본 운영 액션 |
| :--- | :--- | :--- |
| `fresh` | 최신성 기준 충족 | 기본 표기 |
| `delayed` | 허용 범위 내 지연 | 경고 배지 + 자동 재조회 유지 |
| `stale` | 임계치 초과 | 핵심 카드에서 경고, 수동 재조회 노출 |
| `failed` | 최신 조회 실패 또는 정상 스냅샷 부재 | 오류 이유와 fallback/빈 상태 분리 표기 |
| `manual-only` | 자동 freshness 대상이 아닌 수동 점검 데이터 | 자동 경고 제외, runbook/수동 명령 연결 |

### 2.4 OCI 원격 접근 운영 표준 (v3.3-3, 31-02)
운영 데이터와 컨테이너 source of truth는 OCI VM이다. 따라서 WSL/로컬에서 운영 명령을 실행할 때도, 가능하면 OCI를 원격 실행 대상으로 사용한다.

운영 규칙:
1. 운영 `docker compose`, `docker exec`, monitoring script, 운영 SQL 조회는 OCI에서 실행하는 것을 기본값으로 한다.
2. WSL/로컬에서는 SSH wrapper(`scripts/ops/oci_remote_exec.sh`)와 터널 wrapper(`scripts/ops/oci_tunnel.sh`)를 표준 접근 경로로 사용한다.
3. 로컬 Docker/WSL integration 유무는 개발 편의 사항일 뿐, 운영 접근 prerequisite가 아니다.
4. 실제 호스트/키/포트 값은 repo에 커밋하지 않고 `deploy/cloud/oci/ops/oci_access.env` 같은 로컬 비밀 설정에서만 관리한다.
...
**심볼 비중 재배분 정책 (v3.3-1, 29번 핫픽스)**
| 심볼 | 배율 | 목적 |
| :--- | :--- | :--- |
| KRW-BTC | 1.2 | 전환 구간에서 상대적으로 안정적인 기준 자산 비중 확대 |
| KRW-ETH | 1.2 | 구조적 유동성/시총 상위 자산 비중 확대 |
| KRW-SOL | 1.2 | 추세 추종 민감 구간의 기회 유지 |
| KRW-XRP | 0.7 | 손익 변동성 대비 기대수익 열위 구간 비중 축소 |
| KRW-DOGE | 0.7 | 변동성 과대 구간 노출 축소 |

적용 원칙:
1. 레짐 비중(`position_size_ratio`)과 심볼 배율(`symbol_position_multiplier`)은 **동적 하드 캡 안에서의 목표 주문량 조정 계수**로만 사용한다.
2. 최종 목표 주문금액은 `기준자산 × max_per_order × volatility_multiplier × min(position_size_ratio × symbol_multiplier, 1.0)`로 계산한다.
3. 배율 합 `5.0`(=1.2+1.2+1.2+0.7+0.7)으로 기존 `1.0*5`와 총량을 맞춰 전체 리스크 레벨 급변을 방지한다.
4. 미정의/비정상 배율은 `1.0`으로 폴백해 운영 안정성을 우선한다.
...
보조 원칙:
1. `max_per_order`는 "종목당 최대 허용 비중"의 의미를 유지한다.
2. `position_size_ratio`, `symbol_position_multiplier`는 하드 캡을 깨는 배율이 아니라, 캡 안에서 목표 주문량을 줄이거나 유지하는 조정 계수다.
3. `volatility_multiplier`는 고변동성 구간의 보수 장치이며, 목표 주문량 계산과 최종 리스크 검증에 동일하게 적용한다.
...
| **LLM** | Claude Haiku 4.5 (Primary) + GPT-4o-mini (AI Decision Canary) | 기본 경로 안정성 유지 + 제한적 카나리 실험으로 비용/품질 비교 |
...
| `docs/work-plans/18-17_trade_notification_payload_fallback_and_color_split_plan.md` | 18-17 Trade Notification payload fallback 및 BUY/SELL 색상 분기 계획 |
...
| `docs/work-plans/29_regime_transition_strategy_evaluation_and_hotfix_plan.md` | 29 레짐 전환 구간 전략 성능을 정량 검증하고 조건부 핫픽스 여부를 결정하는 계획 |
| `docs/work-plans/22_dashboard_readability_and_live_data_reliability_plan.md` | 22 대시보드 가독성/실시간성 요구사항을 프론트 무관 spec으로 표준화하는 계획 |
| `docs/work-plans/31-02_oci_remote_ops_access_standardization_plan.md` | 31-02 WSL에서 OCI 원격 실행/터널을 표준화해 운영 접근 경로를 단일화하는 계획 |
| `docs/work-plans/23_nextjs_dashboard_gradual_migration_plan.md` | 23 Next.js 점진 이관(모노레포 시작, 병행 운영, 단계적 전환) 계획 |
| `docs/work-plans/23-01_frontend_backend_repository_split_timing_plan.md` | 23-01 프론트/백엔드 레포 분리 시점/게이트/절차를 정의하는 하위 계획 |
...
| `docs/work-plans/24_discord_mobile_chatbot_query_plan.md` | 24 Discord 모바일 조회/대화형 챗봇 구축 계획 |
...
| `docs/work-result/31-02_oci_remote_ops_access_standardization_result.md` | 31-02 OCI 원격 운영 접근 wrapper/터널 표준화 결과 |
| `docs/work-result/24_discord_mobile_chatbot_query_result.md` | 24 Discord 모바일 조회/대화형 챗봇(읽기 전용 Slash Command + 내부 Mobile API) 구현 결과 |
...
| `docs/troubleshooting/24_mobile_visibility_gap_discord_query_need.md` | 24 모바일 운영 조회 공백(단방향 알림 한계)으로 Discord 조회형 챗봇을 도입한 배경/원인/대응 기록 |
| `docs/troubleshooting/24-01_discord_role_nonetype_guard_fix.md` | 24-01 Discord Slash Command 권한검사에서 `NoneType role.id` 예외로 발생한 무응답 이슈 대응 기록 |
| `docs/troubleshooting/24-02_mobile_api_500_missing_psycopg2.md` | 24-02 Discord `/positions` 호출 시 Mobile API 500(동기 DB 드라이버 누락) 이슈 대응 기록 |
...
5. Major 구현 완료 시 README를 같은 변경 단위에서 갱신하고, 결과 문서에서 동기화 검증 명령을 남긴다.
6. Result/Troubleshooting은 "문제 정의 + before/after 정량 증빙 + 측정 근거 명령"이 없으면 완료(`done`)로 간주하지 않는다.
...
| 2026-03-01 | 24 모바일 운영 채널 확장: Discord 읽기 전용 Slash Command(`/positions`, `/pnl`, `/status`, `/risk`, `/ask`)와 내부 Mobile API(`/api/mobile/*`)를 추가하고, `X-Api-Secret` 인증/allowlist/rate-limit 가드레일을 반영 |
| 2026-03-01 | 24 트러블슈팅 기록 추가: 모바일 운영 조회 공백(단방향 알림 한계) 이슈의 원인/영향/대응 배경을 별도 문서로 기록하고 Plan/Result와 상호 참조 연결 |
| 2026-03-02 | 24-01 운영 핫픽스 반영: Discord 권한검사 로직의 `NoneType role.id` 예외를 방어하고(guild intent 보강, payload fallback, 명령 전역 에러 핸들러), Slash Command 무응답 이슈를 완화 |
| 2026-03-02 | 24-02 운영 핫픽스 반영: Mobile API 조회 경로에서 `psycopg2` 누락으로 발생한 500 이슈를 해결하기 위해 bot 런타임 의존성(`psycopg2-binary`)을 추가 |
| 2026-03-02 | 27 CI 복구 반영: `security` 잡의 requirements 동시 설치 충돌을 제거하고(`pip-audit` 분리 실행), `test` 잡에 DB fail-fast 대응 env(`DB_PASSWORD`, `DATABASE_URL`)를 주입해 collection 오류를 해소 |
| 2026-03-02 | 27-01 보안 후속 반영: RSS XML 파싱을 `defusedxml`로 전환(B314), bot 바인딩 주소를 env 주입(`BOT_HOST`)으로 변경(B104), security report 파일 선생성으로 artifact 경고를 완화 |
| 2026-03-02 | 27-02 보안 후속 반영: 취약 가능 의존성 상향(`fastapi/httpx/redis/uvicorn/streamlit/plotly` 등), CI `pip-audit` 상세 요약 게이트 추가, 즉시 해소 불가 CVE는 `security/pip_audit_ignored_vulns.txt` allowlist로 제한 예외 처리(문서화/재검토 전제) |
| 2026-03-02 | 27-03 착수: 운영 OCI와 분리된 `f27`/WSL 실험으로 backend-agent 취약점 우선 정리. core/bot `langgraph`를 `0.6.11`로 정렬해 checkpoint 구버전 유입을 줄이고, `streamlit` 충돌 방지를 위해 `pillow>=12.1.1` 직접 핀을 제거 |
| 2026-03-02 | 27-03 보강: CI security 노이즈 감축을 위해 `pip-audit` 개별 step는 종료코드 파일만 기록하고, 최종 요약 step에서만 pass/fail을 단일 판정하도록 조정 |
| 2026-03-03 | 25-01 운영 규칙 반영: Major 완료 시 README 동기화 의무를 문서 워크플로우에 고정하고, 비프로젝트성 관리/메타 작업 번호를 `99-` prefix로 표준화 |
| 2026-03-04 | 21-03 카나리 구현 반영: AI Decision(Analyst/Guardian)에 provider/model 카나리 라우팅(신호 해시 deterministic 분기, 0~20% 상한, canary key 누락 시 primary fallback)을 추가하고, 모델별 운영 집계 스크립트(`scripts/ops/ai_decision_canary_report.sh`)를 도입 |
| 2026-03-04 | 21-04 관측성 확장(Phase 1): LLM 호출 원장(`llm_usage_events`)과 크레딧 스냅샷(`llm_credit_snapshots`) 스키마를 추가하고, AI Decision/Chatbot/Daily Report/RAG embedding(estimated) 경로 usage 계측 및 운영 리포트 스크립트(`scripts/ops/llm_usage_cost_report.sh`)를 도입 |
| 2026-03-04 | 99-01 운영 문서 품질 게이트 반영: Result/Troubleshooting에 문제 정의 + before/after 정량 증빙 + 측정 근거(명령/데이터 출처) 의무화를 Charter/AGENTS/템플릿에 고정 |
| 2026-03-05 | 21-04 관측성 확장(Phase 2 코드): `LLM_CREDIT_SNAPSHOT_*` env 기반 provider API 크레딧 스냅샷 수집 로직과 scheduler 주기 실행을 추가하고, one-shot 수집 스크립트(`scripts/ops/llm_credit_snapshot_collect.sh`) 및 리포트 freshness 구간을 보강 |
| 2026-03-05 | 21-04 관측성 확장(Phase 2.1 코드): provider balance endpoint 의존을 줄이기 위해 `LLM_COST_SNAPSHOT_*` 설정과 `llm_provider_cost_snapshots` 스키마를 추가하고, reconciliation 축을 `provider_cost_usd`(외부 비용 합계) vs `ledger_cost_usd`(내부 원장 합계)로 전환 |
| 2026-03-05 | 21-05 가독성 보강(Phase 4): cAdvisor 라벨 공백 환경을 우회하기 위해 `coinpilot-container-map` 사이드카(docker ps→node-exporter textfile metric)를 추가하고, 인프라 대시보드 컨테이너 패널을 `coinpilot_container_display_info` 조인 + 12자리 ID fallback으로 전환 |
| 2026-03-05 | 21-05 핫픽스(Phase 4-1): `docker_only=true` 환경에서 cAdvisor `id` 포맷(`/docker/<id>`) 변화로 최근 구간 `No data`가 발생하는 케이스를 반영해, 패널 정규식을 `docker-`/`docker/` 겸용으로 보강 |
| 2026-03-05 | 21-05 핫픽스(Phase 4-2): `container-map` 조인 구조를 유지하면서 cAdvisor를 `docker_only=false`로 재조정해 최근 구간(5m/15m) 시계열 공백 재발을 완화 |
| 2026-03-05 | 21-05 핫픽스(Phase 4-3): cAdvisor가 `id="/"`만 노출하는 환경을 우회하기 위해 `container-map`에 CPU/Memory/Restart 메트릭 생성을 추가하고, 컨테이너 패널 데이터 소스를 해당 메트릭으로 전환 |
| 2026-03-05 | 22/23 계획 정렬: 22를 Streamlit 직접 리팩터가 아닌 프론트 무관 Spec-First로 재정의하고, 23은 모노레포 시작 후 안정화 시 FE 분리(23-01 게이트 기반) 전략으로 조정 |
| 2026-03-05 | 아키텍처 경계 명시: 프론트 이관(23)과 AI/Backend 도메인 책임(매매/리스크/LLM 판단)을 분리해 변경 리스크를 축소하는 운영 원칙을 계획 문서에 반영 |
| 2026-03-05 | 실행 우선순위 정책 보강: FE/BE 이관(23)은 `21-03/21-04/28/22/29` 선행 게이트 충족 전 `blocked`로 유지하고, 비이관 트랙(관측/실험/spec) 완료 후 착수하도록 체크리스트와 동기화 |
| 2026-03-05 | 29 계획 추가: 레짐 전환 구간에서 전략 기회손실/과보수성 가설을 검증하기 위한 운영 데이터 감사 + 백테스트 + 조건부 핫픽스 게이트를 정의 |
| 2026-03-06 | 29 핫픽스 반영: 심볼 비중 재배분 정책(BTC/ETH/SOL 1.2, XRP/DOGE 0.7)을 전략 공통 설정으로 승격하고, 실거래(`src/bot/main.py`)와 백테스트(`scripts/backtest_v3.py`) 계산식을 동일 규칙으로 정렬 |
| 2026-03-06 | 21-07 착수 반영: OCI 운영 로그 관측 표준으로 Loki/Promtail/Grafana 연동(Compose 서비스, datasource provisioning, 24h 점검 스크립트 T+1h 로그 파이프라인 체크)을 추가 |
| 2026-03-06 | 21-07 운영 핫픽스 1차: promtail docker_sd의 Docker API mismatch(`1.42 < min 1.44`) 완화를 위해 `PROMTAIL_DOCKER_API_VERSION` 설정과 점검 스크립트 FAIL 기준 강화를 추가 |
| 2026-03-06 | 21-07 운영 핫픽스 2차: 1차 조치 후 mismatch가 재현되어 `promtail-targets`(로그 symlink 생성) + promtail 파일 타깃 수집 구조로 전환해 Docker API 직접 의존을 제거 |
| 2026-03-06 | 21-07 운영 보강 3차: 파일 타깃 수집에서 `service` 라벨 누락 WARN을 줄이기 위해 promtail 라벨 추출을 relabel(`__path__`)에서 pipeline(`filename`) 기반으로 전환 |
| 2026-03-06 | 21-07 운영 보강 4차: `timestamp too old`/tailer churn 재발을 완화하기 위해 promtail-targets를 증분 갱신 방식으로 전환하고 promtail positions 영속화 + t1h 오류 분류(too old WARN)를 추가 |
| 2026-03-06 | 21-07 마감 반영: OCI 재검증에서 `check_24h_monitoring.sh t1h` `FAIL:0/WARN:2`와 Loki ingest(`sum(count_over_time(...[5m]))=1362`)를 확인해 로그 관측 체계를 `done`으로 전환하고 README/체크리스트/결과 문서를 동기화 |
| 2026-03-06 | 21-08 착수 반영: `coinpilot-infra` 대시보드에 Loki 로그 패널(ingest/top talker/pipeline error/too old/api mismatch) 5종을 추가하고, runbook 로그 정상 기준을 `service` 라벨 중심에서 `filename` 기반 ingest 쿼리 기준으로 정렬 |
| 2026-03-07 | 21-08 마감 반영: OCI 런타임 검증에서 `check_24h_monitoring.sh t1h` `FAIL:0/WARN:1`, Loki ingest(`sum(count_over_time({filename=~\"/targets/logs/coinpilot-.*\\\\.log\"}[5m]))=187`)를 확인해 Grafana 로그 패널화 작업을 `done`으로 확정하고 README/체크리스트/결과 문서를 동기화 |
| 2026-03-07 | 21-08 후속 운영 기준 반영: `coinpilot-infra` 패널 description에 주의/위험 기준을 명시하고, 절대 기준이 유효한 9개 패널에 threshold를 적용해 정량 운영 판단 기준을 대시보드에 내장 |
| 2026-03-07 | 21-08 후속 알림 코드화 반영: Grafana alert rule 7개(core/resource/log pipeline)를 provisioning YAML로 추가하고, compose에 `/etc/grafana/provisioning/alerting` 마운트를 연결해 재기동 시 규칙 자동 로드를 표준화 |
| 2026-03-07 | 21-08 후속 알림 안정화 반영: Loki alert rule 3개에 `or vector(0)` + `noDataState=OK`를 적용하고, API mismatch 규칙을 `$B >= 3`, `for:5m`로 완화해 재기동 직후 Pending/No data 노이즈를 축소 |
| 2026-03-10 | 31 마감 반영: `/etc/cron.d/coinpilot-monitoring` + `run_scheduled_monitoring.sh` 표준을 운영 정책으로 확정하고, `check_24h_monitoring.sh`에 `LLM snapshot freshness`/`AI decision inactivity`/`scheduled heartbeat` 자동 판정을 추가했다. OCI 검증에서 `all --automation-mode` `FAIL:0/WARN:0`을 확인해 cron 자동화 + 관측 갭 보강을 `done`으로 전환하고 README/체크리스트/결과 문서를 동기화 |
| 2026-03-11 | 21-10 정책 정렬 반영: 주문 목표 계산과 RiskManager 하드 캡 검증을 동일 정책으로 정렬하고, `max_per_order`를 종목당 최대 허용 비중으로 유지한 채 `position_size_ratio`/`symbol_position_multiplier`를 동적 하드 캡 안의 목표 주문량 조정 계수로 재정의 |
| 2026-03-12 | 22 운영 UI 표준 반영: 대시보드 기술 스택과 무관한 정보 우선순위(Primary/Secondary/Diagnostic), freshness 상태 5종(`fresh/delayed/stale/failed/manual-only`), monitoring-only/수동 검증 데이터의 배치 원칙을 Charter 운영 규칙으로 승격했다. 관련: `docs/work-plans/22_dashboard_readability_and_live_data_reliability_plan.md`, `docs/work-result/22_dashboard_readability_and_live_data_reliability_result.md` |
| 2026-03-12 | 31-02 OCI 접근 표준 반영: WSL/로컬에서 운영 명령을 실행할 때도 OCI를 source of truth로 쓰도록 `oci_remote_exec.sh`/`oci_tunnel.sh` wrapper와 `oci_access.env.example`를 추가하고, runbook/README를 같은 기준으로 동기화했다. 관련: `docs/work-plans/31-02_oci_remote_ops_access_standardization_plan.md`, `docs/work-result/31-02_oci_remote_ops_access_standardization_result.md` |

---
*최종 업데이트: 2026-03-12 (31-02 반영: OCI 원격 접근 운영 표준 확정) by Codex (GPT-5)*
