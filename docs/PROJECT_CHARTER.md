...
| 2026-03-02 | 27-02 보안 후속 반영: 취약 가능 의존성 상향(`fastapi/httpx/redis/uvicorn/streamlit/plotly` 등), CI `pip-audit` 상세 요약 게이트 추가, 즉시 해소 불가 CVE는 `security/pip_audit_ignored_vulns.txt` allowlist로 제한 예외 처리(문서화/재검토 전제). 추가로 `streamlit`-`pillow` 충돌에 따라 `pillow>=12.1.1` 직접 pin 상향은 철회하고 해당 CVE를 allowlist로 전환 |

---
*최종 업데이트: 2026-03-02 (27-02 반영: pip-audit 취약점 대응 + allowlist 예외 관리 + pillow 충돌 조정) by Codex (GPT-5)*
