...
적용 원칙:
1. 레짐 비중(`position_size_ratio`)과 심볼 배율을 곱해 최종 주문 비중을 계산한다.
2. 배율 합 `5.0`(=1.2+1.2+1.2+0.7+0.7)으로 기존 `1.0*5`와 총량을 맞춰 전체 리스크 레벨 급변을 방지한다.
3. 미정의/비정상 배율은 `1.0`으로 폴백해 운영 안정성을 우선한다.
...
---
*최종 업데이트: 2026-03-10 (31 마감: cron 자동화 + 관측 갭 보강 반영) by Codex (GPT-5)*
